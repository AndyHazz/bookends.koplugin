...
>
> The data comes from the `metadata.calibre` file Calibre writes when it sends books to the device, and Bookends looks for it in **KOReader's home folder** (the one set under *Settings > Home folder*). If your books live somewhere else, or you copy files across by hand instead of sending them from Calibre, there is no such file and these tokens stay empty. A book with no value for a column shows nothing rather than a blank space, so `[if:calibre{pubdate}]%calibre{pubdate}[/if]` hides itself on books that have no date.
